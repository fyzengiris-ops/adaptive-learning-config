...
推荐用一个阅读 Shell 包住原型页面根容器（写在 `index.html` + `js/prd/requirement-reader-shell.js`）：

```html
<div id="prd-reader-shell" class="prd-reader-shell">
  <div class="prd-prototype-area">
    <!-- 原有原型页面内容 -->
  </div>
  <aside class="prd-panel" hidden>
    <!-- 右侧 PRD 阅读面板 -->
  </aside>
</div>
```
...
如果原型页面内部有 `position: fixed; right: 0` 一类元素，打开 PRD 面板后会贴到浏览器最右侧并覆盖 PRD 面板。
...
具体项目中可根据布局选择容器定位或 CSS 变量补偿。对筛选抽屉、课程目录等右侧浮层，优先保证它出现在原型区域内，不要和 PRD 阅读面板重叠。
...
async function activateRequirement(requirement) {
  ...
  for (const step of requirement.activate) {
    switch (step.type) {
      case 'navigate':
        // 设置 location.hash，例如 #chapter；已在目标页则不重复跳转。
        break;
      case 'openPanel':
        // 调用页面提供的显式控制器，例如 openFilterDrawer。
        break;
      case 'openDialog':
        // 调用页面提供的显式控制器，例如 openCourseCenterModal。
        break;
      case 'setTab':
      case 'setStep':
        // 设置页面状态，例如 switchChapterType('extend')。
        break;
      case 'scrollTo':
...
- 优先使用显式状态控制器（优先复用 `js/app.js` 已有函数）。
- 不要依赖按钮文案模拟点击。
- 需要等待弹窗、抽屉或 hash 视图挂载后，再滚动和高亮。
...
```js
requirement.logicSections.forEach((section) => {
  // section.title
  // section.items → 单层 1.1 / 1.2 编号
});
```
...
- 二次套用固定子模板栏目
- 自动补空栏目
- 读取旧 `display` / `operation` 作为主正文
- 双编号

## 过滤空兜底说明

若 `items` 中出现下列空兜底，默认不展示：
