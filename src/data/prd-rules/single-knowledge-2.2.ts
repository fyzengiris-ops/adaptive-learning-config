/**
 * 单知识点出题 2.2 详情页-查看通用策略详情
 * 覆盖范围：页面头部（查看态）、状态提示区、基础信息区（只读）、题目组成配置区（只读）、底部操作区（编辑/删除）、发布操作、历史发布记录、策略不存在
 * 去重原则：同一条规则只归入一个分类，不重复描述；与2.0新增态不重复（2.0覆盖新增/编辑态规则；本文件仅覆盖查看态规则）
 */
...
export const pageHeader: PrdTooltipData = {
  title: '页面头部',
  rules: [
    { category: '字段规则', content: '查看态页面标题固定文案【通用策略详情】' },
...
export const historyEntry: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '交互规则', content: '点击后打开当前策略的历史发布记录弹窗' },
...
export const pendingPublishBar: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '交互规则', content: '点击【发布】→ 打开发布确认弹窗' },
  ],
};
...
export const basicInfoSection: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '显示规则', content: '查看态下学段和学科均为纯文本展示，不可编辑，不展示下拉控件' },
    { category: '显示规则', content: '通用策略无策略名称字段' },
  ],
};
...
export const emptyConfigState: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '交互规则', content: '用户可点击【编辑】进入编辑态后重新添加区间' },
  ],
};
...
export const editAction: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '交互规则', content: '点击后进入编辑态' },
...
export const deleteStrategy: PrdTooltipData = {
  ...
  rules: [
    ...
    { category: '交互规则', content: '弹窗内容：确认删除【学段学科】的单知识点出题策略吗？（个性化策略格式为【学段学科-策略名称】）' },
    ...
    { category: '数据规则', content: '删除后下游业务立即无法获取该通用策略' },
  ],
};
